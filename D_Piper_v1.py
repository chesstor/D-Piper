...
with open('D_Piper_v1_options.txt', "r") as File_Opciones:
    ...
    for Registro in csv:
        ...
        if Registro[0] == 'WhatGroupToDraw':
            WathGroupToDraw = Registro[2].split(',')
        if Registro[0] == 'Identificate':
            Identificar = bool_conv(Registro[2])
        if Registro[0] == 'LabelColor':
            LabelColor = Registro[2]
        if Registro[0]== 'Combinate':
            Combinate = bool_conv(Registro[2])
...
D.groups_toDraw = WathGroupToDraw     # Selected groups to be drawn
...
#################------------ Standard Piper -------------#####################
D.s_piper_group(WathGroupToDraw) if Separate_Groups == True else D.s_piper()
#################------------ Density Piper -------------#####################
...
del [p_title, d_title, bins,  pt_size, d_transparency, p_transparency, Color_Unico,
     ColorMap, SetColores, WathGroupToDraw, Identificar, LabelColor, Transform_Log,
     ...
     csv, File_Opciones, Combinate, ov_title, Grid]
